...
def simulate_closed_loop(traj, car_model, controller, FF_type=None, FF_model=None, FF_input=None, dt=0.05):
    ...
    Returns:
    log_FF_data: pd.DataFrame, data from the FF model
    log_traj_data: pd.DataFrame, data from the trajectory

    """
    time = traj['time']

    log_traj_data = {'time': [], 'x': [], 'y': [], 'psi': [], 'vl': [], 
    ...
    for i, t in enumerate(time):
        ...
        x_a, y_a, vl_a, psi_a = car_model.state
        ...
        u_steer_FB, u_speed_FB = controller.control(x_a=x_a, x_d=x_d, 
                                                    y_a=y_a, y_d=y_d, 
        ...
        log_traj_data['x'].append(car_model.state[0])
        log_traj_data['y'].append(car_model.state[1])
        log_traj_data['psi'].append(car_model.state[2])
    ...
    return pd.DataFrame(log_traj_data)
...
def FF_yawrate_ctrl(desired_curvature: float, L: float) -> float:
    """ Returns the FF steering angle """
    delta = np.arctan(L * desired_curvature)
    return delta

def FB_yawrate_ctrl(yaw_rate_desired: float, yaw_rate_actual: float, ctrl, t: float) -> float:
    """ 
    Args:
        yaw_rate_desired: float, desired yaw rate
        yaw_rate_actual: float, actual yaw rate
        ctrl: PID controller
        t: float, time
    ...
    Returns:
        u, control input
    """
    error = yaw_rate_actual - yaw_rate_desired
    u = ctrl.control(error, t=t)
    return -u